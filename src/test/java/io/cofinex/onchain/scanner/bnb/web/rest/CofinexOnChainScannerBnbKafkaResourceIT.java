package io.cofinex.onchain.scanner.bnb.web.rest;
...
import io.cofinex.onchain.scanner.bnb.IntegrationTest;
import io.cofinex.onchain.scanner.bnb.config.EmbeddedKafka;
import io.cofinex.onchain.scanner.bnb.config.KafkaSseConsumer;
import io.cofinex.onchain.scanner.bnb.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class CofinexOnChainScannerBnbKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cofinex-on-chain-scanner-bnb-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/cofinex-on-chain-scanner-bnb-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/cofinex-on-chain-scanner-bnb-kafka/unregister"));
                return;
            }
        }
